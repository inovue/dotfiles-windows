...
- **Graph present**: `graphify-out/graph.json` exists — graph-first navigation applies (survey: `just audit` + hubs; modify: `just path` + scoped `rg -n`). A post-commit hook auto-rebuilds the graph on every commit.
...
scripts/           01_winget, 02_fonts, 03_runtimes, 04_configs, audit_workspace, sync_agent_rules, setup_api_keys, agent_guard.py
tests/             verify_tools.ps1 (full environment test suite)
...
| `just test` | full environment test suite |
