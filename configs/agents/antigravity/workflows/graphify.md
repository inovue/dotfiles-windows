...
5. **After edits** → `just update-graph` (= `graphify update .`) once per batch (WaitMs ≥ 60000 if needed) before concluding. The post-commit hook (`just install-graph-hook`) additionally auto-rebuilds the graph on every commit.
