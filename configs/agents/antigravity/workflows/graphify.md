...
5. **After edits** → `just update-graph` (= `graphify update .`) once per batch, then `just deploy` (if configs changed) and `just audit`. WaitMs ≥ 60000 if the graph update needs it. The post-commit hook (`just install-graph-hook`) additionally auto-rebuilds the graph on every commit.
