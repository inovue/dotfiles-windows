...
sync-rules:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -File ./scripts/sync_agent_rules.ps1

# Check if all AI Agent rules and skills are in sync with the master SSOT files
check-rules:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -File ./scripts/sync_agent_rules.ps1 -Check

# Deploy all dotfile configurations (Windows Terminal, Helix, Nushell, Profiles, etc.)
deploy:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -File ./scripts/04_setup_configs.ps1

# Run the comprehensive test suite to verify CLI tools, environment variables, and rules
test:
    @powershell.exe -NoProfile -ExecutionPolicy Bypass -File ./tests/verify_tools.ps1
